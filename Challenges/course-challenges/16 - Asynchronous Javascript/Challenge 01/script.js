...
the attributes that you recieved about the provided location. T
hen, using this data, log a messsage like this to the console: 'You are in Berlin, 
Germany'
...
you will get this error with code 403. This is an error with the request.
 Remember, fetch() does NOT reject the promise in this case. So create an error to
...
const whereAmI = (lat, long) => {
  fetch(`https://geocode.xyz/${lat},${long}?geoit=json`)
    .then((response) => {
      if (!response.ok)
        throw new Error(`Error with geocoding ${response.status}`);
    ...
    .then((response) => {
      if (!response.ok) throw new Error(`Country doesn't exist`);
